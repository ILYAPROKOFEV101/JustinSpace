    ...
    public partial class MainWindow : Window
    {
        ...
        private void StartButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Получаем значение топлива из текстового поля
                if (!int.TryParse(FuelInput.Text, out int fuel))
                {
                    MessageBox.Show("Пожалуйста, введите корректное количество топлива.", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }

                if (fuel <= 0)
                {
                    MessageBox.Show("Количество топлива должно быть больше 0.", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                double liftHeight = Math.Min(fuel * 2, 200); // Максимальная высота - 200
                ...
                Console.WriteLine($"StartButton_Click: Создана анимация подъема на {liftHeight} единиц.");
